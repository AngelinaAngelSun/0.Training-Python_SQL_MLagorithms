...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием IN)

SELECT 
		count(c.city_name)
FROM shipping.city c 
WHERE c.population > ( select avg(cc.population)
                       from shipping.city cc )
...
Оставьте почтовые индексы, в которых хотя бы один из этих показателей ненулевой. 
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием IN-запроса):

SELECT 
	DISTINCT c.state
FROM  
	shipping.city c 
WHERE c.city_id IN    ( SELECT 
				s.city_id
			FROM  
				shipping.shipment s)  
ORDER BY 1; 
...
2) ВАРИАНТ РЕШЕНИЯ (с использованием EXCEPT-запроса)
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием EXISTS-запроса)
...
WHERE EXISTS (
                    select *  
                    from shipping.driver d  
                    where c.city_id = d.city_id and d.phone is not null
                    )
ORDER BY 1



3) ТРЕТИЙ ВАРИАНТ РЕШЕНИЯ (с использованием join)

SELECT  
      DISTINCT c.state
FROM shipping.city c  
    JOIN shipping.driver d ON c.city_id = d.city_id AND d.phone IS NOT NULL
ORDER BY 1
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием CTE-запроса)
...
WITH new_table AS 
               (select 
                ...
                from shipping.driver d  
                    join shipping.shipment s on s.driver_id = d.driver_id
                group by d.driver_id)
SELECT 
        AVG(t.avg_weight) as avg_avg_weight      
FROM new_table as t 
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (простой запрос без SELECT FROM SELECT)

SELECT 
        d.first_name,
        c.cust_name,
        count(s.ship_id) as ships_cnt
FROM shipping.shipment s 
      JOIN shipping.driver d on d.driver_id = s.driver_id
          JOIN shipping.customer c on c.cust_id = s.cust_id  
GROUP BY d.first_name, c.cust_name
ORDER BY ships_cnt DESC       
LIMIT 1
...
1) ПЕРВЫЙ ВАРИАНТ (с использованием оконных функций)
...
FROM 
       ( select distinct
               ...
               d.driver_id,
               c.cust_name,
               count(s.ship_id) over(partition by d.first_name, c.cust_name) as ships_cnt,
               count(s.ship_id) over(partition by d.first_name) as total_cnt
         from shipping.shipment s 
             join shipping.driver d on d.driver_id = s.driver_id
                 join shipping.customer c on c.cust_id = s.cust_id  
         order by ships_cnt desc 
...
FROM 
        ( select 
          ...
          from shipping.shipment s 
                join shipping.driver d on d.driver_id = s.driver_id
                    join shipping.customer c on c.cust_id = s.cust_id  
          group by d.first_name, d.driver_id, c.cust_name
          order by ships_cnt desc 
          limit 1 ) a  
   JOIN shipping.shipment sh on sh.driver_id = a.driver_id
GROUP BY 1, 2, 3
...
1) ПЕРВЫЙ ВАРИАНТ РЕШЕНИЯ (с использованием оконных функций и CTE-запросов - НАИБОЛЕЕ ЭФФЕКТИВНЫЙ)
 
with new_table_1 as (select 
                          distinct
                              d.first_name,
                              cus.cust_name,
                              count(s.ship_id) over(partition by d.first_name, cus.cust_name) as ships_cnt,
                              max(s.ship_date) over(partition by d.first_name, cus.cust_name) as max_date,
                              count(s.ship_id) over(partition by d.first_name) as total_ships_cnt
                    from shipping.shipment s 
                         join shipping.driver d on d.driver_id = s.driver_id
                             join shipping.customer cus on cus.cust_id = s.cust_id 
                    order by ships_cnt desc 
                    limit 1),
  new_table_2 as  (select 
                            d.first_name,
                            count(distinct tr.truck_id) as trucks_cnt
                   from shipping.shipment sh 
                       join shipping.driver d on d.driver_id = sh.driver_id  
                           join shipping.truck tr on tr.truck_id = sh.truck_id
                               join new_table_1 t_1 on t_1.first_name = d.first_name
...
select 
        ...
        t_1.total_ships_cnt,
        t_2.trucks_cnt
from new_table_1 as t_1 
     join new_table_2 as t_2  on  t_2.first_name = t_1.first_name



2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием оконных функций и SELECT FROM SELECT-запросов)

SELECT  new_table_1.first_name,
        new_table_1.cust_name,
        new_table_1.max_date,
        new_table_1.total_ships_cnt,
        new_table_2.trucks_cnt
FROM
      (select 
            distinct
                d.first_name,
  ...
  JOIN  
       (select 
        ...
        from shipping.shipment sh 
            join shipping.driver d on d.driver_id = sh.driver_id  
                join shipping.truck tr on tr.truck_id = sh.truck_id
        group by d.first_name ) as new_table_2   on  new_table_2.first_name = new_table_1.first_name


3) ТРЕТИЙ ВАРИАНТ РЕШЕНИЯ (с использованием GROUP BY и SELECT FROM SELECT-запросоа)
...
1) ПЕРВЫЙ ВАРИАНТ РЕШЕНИЯ (с использованием CTE-запросов - Наиболее эффективный и читабельный)

with    max_driver as ( select 
                                d.first_name,
                                count(s.ship_id) as ship_cnt
                        from shipping.shipment s  
                            join shipping.driver d on d.driver_id = s.driver_id  
                        group by 1  
                        order by 2 desc
                        limit 1), 
        min_driver as (select 
                                d.first_name,
                                count(s.ship_id) as ship_cnt
                        from shipping.shipment s  
                            join shipping.driver d on d.driver_id = s.driver_id  
...
select 
    ...
    max_driver.ship_cnt - min_driver.ship_cnt  as difference
from min_driver, max_driver



2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием SELECT FROM SELECT - запроса)
...
FROM    (select 
        ...
        limit 1) AS min_driver



3) ТРЕТИЙ ВАРИАНТ РЕШЕНИЯ (с использованием типа соединения CROSS JOIN)
...
ЗАДАНИЕ 83.
Выведите идентификатор доставки, ее дату, массу груза, среднюю массу доставленных в этот день грузов 
и признак того, что масса груза больше средней в этот день на 100 кг. Используйте CTE (Common Table Expressions).
...
SELECT 
	...
	s.weight > a.avg_weight + 100  is_heavyweighted
FROM shipping.shipment s
    join a  on  a.ship_date = s.ship_date;


2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с помощью оконных функций)

SELECT
        s.ship_id,
        s.ship_date,
        s.weight,
        avg(s.weight) over(partition by s.ship_date) as  avg_weight_this_day, 
        s.weight > avg(s.weight) over(partition by s.ship_date) + 100 as is_heavyweighted
FROM shipping.shipment  s
ORDER BY 1
...
1) ПЕРВЫЙ ВАРИАНТ РЕШЕНИЯ (с использованием оконных функций и CTE-запросов)
WITH    new_table_1 as  (select
                                s.ship_id,
                                s.ship_date,
                                s.weight,
        ...
        new_table_2 as (select 
                                t.ship_date,
                                count(t.ship_id) as ships_cnt
                        from new_table_1 t 
                        where t.is_heavyweighted
                        group by t.ship_date
                        having count(t.ship_id) > 2)
SELECT 
    'first_day' as date_type,
    min(t2.ship_date) as date
FROM new_table_2 as t2
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием GROUP BY и CTE-запросов)
...
WITH new_table_1 as (select 
    ...
    new_table_2 as  (select 
			    s.ship_date,
			    s.weight > t_1.avg_weight + 100  is_heavyweighted, 
			    count(*) qty
		     from 
			   new_table_1  as t_1
			      join  shipping.shipment s  on  t_1.ship_date = s.ship_date
		     group by 1, 2)
SELECT 
	'first_heavy_date'  as date_type,
	 min(t_2.ship_date)  as date
FROM  new_table_2 as t_2
WHERE   t_2.is_heavyweighted  and  t_2.qty > 2
...
WITH action as  (select
                        ...
                        s.ship_id,
                        s.weight,
                        row_number() over(partition by s.cust_id order by s.weight desc) as weight_number 
                from shipping.shipment s)
SELECT  
        a.cust_id, 
        a.ship_id, 
        ...
        a.weight_number
FROM action AS a    
where a.weight_number in (1,2,3)
order by a.cust_id, a.weight_number  -- where a.weight_number in (1,2,3)
...
SELECT  r.city_name
FROM  rownums r 
WHERE  r.ascending = 1 OR r.descending = 1;
...
1) ПЕРВЫЙ ВАРИАНТ РЕШЕНИЯ (с использованием CTE-запроса)

WITH  new_table as 
                    (select  distinct
                            date_trunc('month', s.ship_date) as ship_date,
                            count(s.ship_id) over(partition by date_trunc('month', s.ship_date)) as ships_cnt,
                            dense_rank() over(order by date_trunc('month', s.ship_date)) as ascending_date,
                            dense_rank() over(order by date_trunc('month', s.ship_date) desc) as descending_date
                    from shipping.shipment s)
SELECT 
        t.ship_date,
        t.ships_cnt
FROM new_table t      
WHERE ascending_date = 1 or descending_date = 1
...
WITH customer_rating as
                        (select distinct
                                c.cust_name,
                                c.annual_revenue,
                                count(s.ship_id) over(partition by c.cust_name) as ships_cnt,
                                avg(s.weight) over(partition by c.cust_name) as avg_weight,
...
where (cr.ascending between 1 and 3) or (cr.descending between 1 and 3)
...
FROM  (select
	...
	order by 2) a  
WHERE a.ascending in (1, 2, 3) OR  a.descending in (1, 2, 3)  
...
ЗАДАНИЕ 96.
Вычислим конверсию из события view в событие addtocart по месяцам.
...
1) ПЕРВЫЙ ВАРИАНТ РЕШЕНИЯ (с использованием оконной функции lead - находит следующее значение в столбце)
/* Функции lead/lag нужны для получения следующих и предыдущих значений в таблице */
...
2) ВТОРОЙ ВАРИАНТ РЕШЕНИЯ (с использованием оконной функции lag - находит предыдущ.значение)
